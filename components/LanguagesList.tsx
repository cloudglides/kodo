...
export function LanguagesList({ languages }: LanguagesListProps) {
  return (
    <div className="bg-primary border border-[#1a1c1a] rounded-lg p-6">
      <h3 className="text-[10px] font-medium text-[#929992] uppercase tracking-wider mb-5" style={{ lineHeight: '10px' }}>Languages</h3>
      
      <div className="space-y-4">
        {languages.slice(0, 5).map((lang, index) => (
          <div key={index}>
            <div className="flex items-center justify-between mb-2 text-sm">
              <span className="text-[#f6fff5] font-medium">{lang.name}</span>
              <span className="text-[#929992] text-[10px] font-medium" style={{ lineHeight: '10px' }}>{lang.text}</span>
            </div>
